...
class RequestMessageLogTest {
    ...
    fun `RequestMessageLog parameterized constructor should set values correctly`() {
        val messageType = RequestMessageLog.MessageType.GAME_THREAD
        val gameId = 123
        val playId = 456
        val messageId = 789L
    ...
    fun `RequestMessageLog parameterized constructor should handle null playId and messageTs`() {
        val messageType = RequestMessageLog.MessageType.PRIVATE_MESSAGE
        val gameId = 123
        val messageId = 789L
        val messageLocation = "test_location"
    ...
    fun `RequestMessageLog properties should be mutable`() {
        ...
        requestMessageLog.messageType = RequestMessageLog.MessageType.GAME_THREAD
        ...
        assertEquals(RequestMessageLog.MessageType.GAME_THREAD, requestMessageLog.messageType)
    ...
    fun `RequestMessageLog should handle both MessageType enum values`() {
        ...
        assertEquals(RequestMessageLog.MessageType.GAME_THREAD, gameThreadLog.messageType)
        assertEquals(RequestMessageLog.MessageType.PRIVATE_MESSAGE, privateMessageLog.messageType)
    }

    @Test
    fun `MessageType enum should have correct descriptions`() {
        assertEquals("Game Thread", RequestMessageLog.MessageType.GAME_THREAD.description)
        assertEquals("Private Message", RequestMessageLog.MessageType.PRIVATE_MESSAGE.description)
    }
    ...
    fun `RequestMessageLog should be instantiable with both constructors`() {
        ...
        val requestMessageLog2 =
            RequestMessageLog(
                RequestMessageLog.MessageType.GAME_THREAD,
                1,
                null,
                1L,
                "location",
    ...
    fun `RequestMessageLog should handle large values`() {
        ...
        assertEquals(RequestMessageLog.MessageType.PRIVATE_MESSAGE, requestMessageLog.messageType)
