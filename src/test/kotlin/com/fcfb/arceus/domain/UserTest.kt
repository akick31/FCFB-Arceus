...
class UserTest {
    ...
    fun `test User parameterized constructor`() {
        ...
        assertEquals(User.CoachPosition.HEAD_COACH, user.position)
    ...
    fun `test User property mutability`() {
        ...
        user.position = User.CoachPosition.OFFENSIVE_COORDINATOR
        user.role = User.Role.CONFERENCE_COMMISSIONER
        user.salt = "newsalt"
        user.team = "New Team"
        ...
        assertEquals(User.CoachPosition.OFFENSIVE_COORDINATOR, user.position)
        assertEquals(User.Role.CONFERENCE_COMMISSIONER, user.role)
    ...
    fun `test CoachPosition enum values`() {
        assertEquals("Head Coach", User.CoachPosition.HEAD_COACH.description)
        assertEquals("Offensive Coordinator", User.CoachPosition.OFFENSIVE_COORDINATOR.description)
        assertEquals("Defensive Coordinator", User.CoachPosition.DEFENSIVE_COORDINATOR.description)
        assertEquals("Retired", User.CoachPosition.RETIRED.description)
    }

    @Test
    fun `test CoachPosition fromString method`() {
        assertEquals(User.CoachPosition.HEAD_COACH, User.CoachPosition.fromString("Head Coach"))
        assertEquals(User.CoachPosition.OFFENSIVE_COORDINATOR, User.CoachPosition.fromString("Offensive Coordinator"))
        assertEquals(User.CoachPosition.DEFENSIVE_COORDINATOR, User.CoachPosition.fromString("Defensive Coordinator"))
        assertEquals(User.CoachPosition.RETIRED, User.CoachPosition.fromString("Retired"))
        assertNull(User.CoachPosition.fromString("Invalid Position"))
    }

    @Test
    fun `test Role enum values`() {
        assertEquals("User", User.Role.USER.description)
        assertEquals("Conference Commissioner", User.Role.CONFERENCE_COMMISSIONER.description)
        assertEquals("Admin", User.Role.ADMIN.description)
    }

    @Test
    fun `test Role fromString method`() {
        assertEquals(User.Role.USER, User.Role.fromString("User"))
        assertEquals(User.Role.CONFERENCE_COMMISSIONER, User.Role.fromString("Conference Commissioner"))
        assertEquals(User.Role.ADMIN, User.Role.fromString("Admin"))
        assertNull(User.Role.fromString("Invalid Role"))
    }
    ...
    fun `test User with all enum values`() {
        ...
        User.CoachPosition.entries.forEach { position ->
            user.position = position
            assertEquals(position, user.position)
        }

        // Test all Role values
        User.Role.entries.forEach { role ->
            user.role = role
