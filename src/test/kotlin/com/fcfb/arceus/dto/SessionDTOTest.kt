...
class SessionDTOTest {
    ...
    fun `Session should be properly annotated`() {
        val entityAnnotation = SessionDTO::class.annotations.find { it is javax.persistence.Entity }
        val tableAnnotation = SessionDTO::class.annotations.find { it is javax.persistence.Table }

        assertNotNull(entityAnnotation, "SessionDTO should be annotated with @Entity")
        assertNotNull(tableAnnotation, "SessionDTO should be annotated with @Table")
    }
    ...
    fun `Session should be a data class`() {
        ...
        assertTrue(session.toString().contains("SessionDTO"))
    }
    ...
    fun `Session should handle different token formats`() {
        ...
        val jwtSession = SessionDTO(1L, jwtToken, expiration)
        val simpleSession = SessionDTO(2L, simpleToken, expiration)
        val longSession = SessionDTO(3L, longToken, expiration)
        val emptySession = SessionDTO(4L, emptyToken, expiration)
    ...
    fun `Session should handle different user IDs`() {
        ...
        val session1 = SessionDTO(1L, "token1", expiration)
        val session2 = SessionDTO(999999L, "token2", expiration)
        val session3 = SessionDTO(0L, "token3", expiration)
    ...
    fun `Session should handle different expiration dates`() {
        ...
        val shortSession = SessionDTO(1L, "token1", shortExpiration)
        val longSession = SessionDTO(2L, "token2", longExpiration)
        val expiredSession = SessionDTO(3L, "token3", pastExpiration)
    ...
    fun `Session data class should support equality comparison`() {
        ...
        val session1 = SessionDTO(1L, 100L, "token", expiration, now)
        val session2 = SessionDTO(1L, 100L, "token", expiration, now)
    ...
    fun `Session should handle session expiration logic`() {
        ...
        val activeSession = SessionDTO(1L, "active-token", futureExpiration)
        val expiredSession = SessionDTO(2L, "expired-token", pastExpiration)
    ...
    fun `Session should handle different ID values`() {
        ...
        val session1 = SessionDTO(1L, 100L, "token1", expiration, LocalDateTime.now())
        val session2 = SessionDTO(Long.MAX_VALUE, 200L, "token2", expiration, LocalDateTime.now())
        val session3 = SessionDTO(0L, 300L, "token3", expiration, LocalDateTime.now())
    ...
    fun `Session should handle token with special characters`() {
        ...
        val session = SessionDTO(1L, tokenWithSpecialChars, expiration)
    ...
    fun `Session constructors should work correctly`() {
        ...
        val fullSession = SessionDTO(1L, 100L, "token1", expiration, now)
        val partialSession = SessionDTO(200L, "token2", expiration)
        val defaultSession = SessionDTO()
    ...
    private fun createTestSession(): SessionDTO {
        val now = LocalDateTime.now()
        val expiration = now.plusHours(24)
